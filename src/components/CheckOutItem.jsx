...
const CheckOutItem = ({ cartItem }) => {
  ...
  return (
    <div key={id} className='flex justify-between items-center border border-solid border-glass p-4 mb-6 '>
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col items-start max-w-[6.8rem]">
          ...
          <div className='flex items-center gap-4 mt-2'>
            <button onClick={() => dispatch(decrease(cartItem))} className='w-8 h-8 text-white bg-black rounded-full'>
              -
            </button>
            <div>{amount}</div>
            <button onClick={() => dispatch(increase(cartItem))} className='w-8 h-8 text-white bg-black rounded-full'>
              +
            </button>
          </div>
        </div>
